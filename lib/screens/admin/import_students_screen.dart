...
class _ImportStudentsScreenState extends State<ImportStudentsScreen> {
  ...
  List<Map<String, dynamic>> parseCsv(String rawCsv) {
    ...
    return rows.skip(1).map((row) {
      ...
      for (int i = 0; i < headers.length; i++) map[headers[i]] = row[i]?.toString();
      return map;
    }).toList();
  }
  ...
  Map<String, dynamic>? mapStudent(Map<String, dynamic> row, List<String> errors) {
    final gr    = row['GR.No']?.trim();
    final name  = row['Name of student']?.trim();
    final phone = row['Contact No']?.trim();
    String? aadhaar = row['Aadhaar No']?.toString().trim();
    aadhaar = aadhaar?.replaceAll(RegExp(r'\D'), '');

    if (gr == null || gr.isEmpty)                     { errors.add("Missing GR");                return null; }
    if (name == null || name.isEmpty)                 { errors.add("Missing Name (GR: $gr)");    return null; }
    if (!RegExp(r'^\d{10}$').hasMatch(phone ?? ''))   { errors.add("Invalid Phone (GR: $gr)");   return null; }
    if (!RegExp(r'^\d{12}$').hasMatch(aadhaar ?? '')) { errors.add("Invalid Aadhaar (GR: $gr)"); return null; }

    final dob       = parseDate(row['Date of Birth']);
    final className = row['Class'];
    final division  = row['Division'];
    String? classId;
    if ((className != null && className.isNotEmpty) && (division != null && division.isNotEmpty)) {
    ...
    return {
      ...
      'address':       row['Address (Pada, Vilage)'],
      'gender':        row['Gender (Male/Female)']?.toLowerCase(),
      'dob':           dob,
      'rollNo':        row['Roll.No']?.isEmpty ?? true ? null : int.tryParse(row['Roll.No']),
      'classId':       classId,
