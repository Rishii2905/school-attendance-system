...
class _LoginPageState extends State<LoginPage>
  ...
  Future<void> _signUp() async {
    ...
    if ([name, phone, email, pass].any((s) => s.isEmpty)) {
    ...
    try {
      ...
      await FirebaseFirestore.instance
          ...
          {'name': name, 'phone': phone, 'email': email, 'role': 'admin'});
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // Prevent the keyboard from resizing the layout — the scroll view
      // inside the bottom card handles keyboard avoidance on its own.
      ...
      body: Stack(children: [
        ...
        LayoutBuilder(builder: (context, constraints) {
          // Cap the header to 28% of screen height — prevents overflow on
          // small screens and when the keyboard is visible.
          final headerH = (constraints.maxHeight * 0.28).clamp(0.0, 195.0);
              ...
              child: FadeTransition(
                ...
                child: SlideTransition(
                  ...
                  child: Container(
                    ...
                    // Wrap in Stack so the dancer strip sits at the very bottom
                    // of the card without participating in scroll layout —
                    // this prevents the overflow when the keyboard is open.
                    child: Stack(
                      children: [
                        // Scrollable form content — bottom padding reserves
                        // space so the last field is never hidden behind the
                        // dancer strip or the system nav bar.
                        ...
                        // Dancer strip pinned to the bottom of the card,
                        // always visible, never scrolls, never overflows.
                        const Positioned(
                          left: 0, right: 0, bottom: 0,
                          child: _WarliDancerBorder(),
                        ),
                      ],
                    ),
                  ),
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        const Text('Only admins can register here',
            style: TextStyle(fontSize: 13, color: _textMuted)),
        const SizedBox(height: 24),
        ...
        if (loading)
          ...
          _BigButton(label: 'Create Admin Account', onPressed: onSignup),
